...
import { Box, VStack, HStack, Text, Grid, GridItem, IconButton, Button } from '@chakra-ui/react';
...
const DeliveryDetails = ({ order, onClose, onSave, onChange }) => {
  ...
  const [selectedDriver, setSelectedDriver] = useState(order.deliveryOrder?.driver?.name || '');
  ...
  const updateParent = useCallback((updatedOrder) => {
  ...
  const formatDate = (date) => date ? format(new Date(date), 'eee MMM d, HH:mm') : 'N/A';
  ...
  const updateRowData = () => {
    const updatedRowData = deliveryOrderAttributes.lineItemsAttributes.map((item) => ({
      id: item.id,
      name: item.name || 'N/A',
  ...
  const handleChange = (field, value) => {
    ...
    const updateOrder = { ...order, deliveryOrderAttributes: updatedDeliveryOrderAttributes };
  ...
  const handleStatusChange = (e) => {
    ...
    deliveryOrder: {
      ...order.deliveryOrder,
      completedAt: newStatus === 'completed' ? new Date().toISOString() : order.deliveryOrder?.completedAt,
    },
  };
  ...
  const saveDriver = (newDriver) => {
    ...
    const selectedDriverObj = drivers.find(driver => driver.name === newDriver);
  ...
  const handleAddLineItem = (newLineItem) => {
    ...
    const updatedLineItems = [...deliveryOrderAttributes.lineItemsAttributes, newLineItem];
    setDeliveryOrderAttributes(prev => ({
      ...prev,
  ...
  const handleEditLineItem = (lineItemId) => {
    ...
    const lineItem = deliveryOrderAttributes.lineItemsAttributes.find(item => item.id === lineItemId);
  ...
  const handleUpdateLineItem = (updatedLineItem) => {
    ...
    const updatedLineItems = deliveryOrderAttributes.lineItemsAttributes.map((item) => 
  ...
  const handleDeleteLineItem = (lineItemId) => {
    ...
    const updatedLineItems = deliveryOrderAttributes.lineItemsAttributes.filter(item => item.id !== lineItemId);
    setDeliveryOrderAttributes(prev => ({
      ...prev,
      lineItemsAttributes: updatedLineItems,
    }));
  ...
  return (
    <ModalWrapper isOpen={true} onClose={onClose} title="Delivery Order" maxWidth={1600} onSave={handleSaveWrapper}>
        ...
        <VStack align="stretch" spacing={4}>
          ...
          <Grid templateColumns="repeat(3, 1fr)" gap={4}>
            <GridItem>
              <VStack align="stretch" bg="gray.700" p={4} borderRadius="md">
                ...
                <InfoItem label="ADDRESS" value={order.customer?.address || "N/A"} />
                <InfoItem label="ZIPCODE" value={order.customer?.zipcode || "N/A"} />
                <InfoItem label="PHONE NO" value={order.customer?.phoneNo || "N/A"} />
            ...
            <GridItem>
              <VStack align="stretch" bg="gray.700" p={4} borderRadius="md">
                ...
                <InfoItem label="CREATED AT" value={formatDate(order.deliveryOrder?.createdAt)} />
                <InfoItem label="STARTED AT" value={formatDate(order.startedAt)} />
                <InfoItem 
                  label="COMPLETED AT" 
                ...
                {editingField === 'driver' ? (
                  <SelectField
                    ...
                    options={drivers?.length > 0 ? drivers.map(d => d.name) : []}
                  />
                ) : (
                  <InfoItem
                    label="DRIVER NAME"
          ...
          {showDetailsForm && (
            <DeliveryDetailsForm
              ...
              onSave={editingLineItem ? handleUpdateLineItem : handleAddLineItem}
